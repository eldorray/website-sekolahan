...
<head>
    ...
    <script>
        (function() {
            ...
            applyTheme();
            // Re-apply after wire:navigate swaps the document
    ...
    <style>
        ...
        .animate-page-in {
            animation: panel-page-in 350ms cubic-bezier(0.16, 1, 0.3, 1) both;
        }

        /* Wire:navigate progress bar (top loading bar) */
        [x-cloak] {
...
<body class="font-sans text-slate-800 min-h-screen relative">
    ...
    <div x-data="{
        ...
        @keydown.escape.window="mobileSidebarOpen = false; profileOpen = false" class="flex min-h-screen">
        {{-- Mobile backdrop --}}
        <div x-show="mobileSidebarOpen" x-transition:enter="ease-ios duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="ease-ios duration-250"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="mobileSidebarOpen = false"
            x-cloak class="lg:hidden fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-md"></div>

        {{-- Sidebar (drawer di mobile, collapsible di desktop) --}}
        <aside
            class="fixed inset-y-0 left-0 z-50 flex flex-col text-white
                   transform transition-all ease-ios duration-400 will-change-transform lg:relative
                   bg-gradient-to-b from-brand-700 via-brand-600 to-brand-800
                   shadow-xl shadow-brand-900/20 overflow-hidden"
            :class="mobileSidebarOpen
                ?
                'translate-x-0 w-72' :
                ('-translate-x-full lg:translate-x-0 lg:m-4 lg:rounded-[2rem] ' + (sidebarOpen ? 'lg:w-64' : 'lg:w-20'))">
            {{-- Decorative Blobs --}}
            <div class="absolute inset-0 pointer-events-none overflow-hidden">
                <div class="absolute -top-20 -right-20 w-64 h-64 bg-brand-300/20 rounded-full blur-3xl"></div>
                <div class="absolute bottom-0 -left-20 w-56 h-56 bg-brand-400/20 rounded-full blur-3xl"></div>
                <div class="absolute top-1/3 right-0 w-32 h-32 bg-amber-300/10 rounded-full blur-2xl"></div>
            </div>

            {{-- Watermark Logo (samar di bagian bawah) --}}
            @if ($logoUrl = \App\Models\Setting::imageUrl('app_logo'))
                <div class="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none overflow-hidden">
                    <img src="{{ $logoUrl }}" alt=""
                        class="w-[420px] h-[420px] object-contain opacity-[0.07] translate-y-20 select-none transition-opacity duration-300"
                        :class="!sidebarOpen && 'lg:opacity-[0.05] lg:w-44 lg:h-44 lg:translate-y-10'">
                </div>
            @endif

            <div class="relative z-10 p-5 border-b border-white/15 flex items-center justify-between backdrop-blur-sm">
                <div class="flex items-center gap-3 overflow-hidden transition-all duration-300"
                    :class="!sidebarOpen && 'lg:opacity-0 lg:w-0'">
                ...
                <div class="hidden lg:flex" :class="!sidebarOpen && 'w-full justify-center'">
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="h-9 w-9 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white apple-transition border border-white/10">
                        <x-icon name="bars-3" class="h-5 w-5" />
                    </button>
                </div>
                <button @click="mobileSidebarOpen = false"
                    class="lg:hidden h-9 w-9 rounded-xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white apple-transition border border-white/10">
                    <x-icon name="x-mark" class="h-5 w-5" />
                </button>
            </div>
            <nav class="relative z-10 p-3 space-y-1 overflow-y-auto flex-1 no-scrollbar">
                @foreach ($nav as $item)
                    @php $active = $current === $item['route']; @endphp
                    <a href="{{ route($item['route']) }}" wire:navigate @click="mobileSidebarOpen = false"
                        class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm apple-transition relative {{ $active ? 'bg-white text-brand-700 shadow-lg shadow-brand-900/20 font-semibold' : 'text-white/85 hover:bg-white/15 hover:text-white' }}"
                        :title="!sidebarOpen ? '{{ $item['label'] }}' : ''">
                        @if ($active)
                            <span
                                class="absolute -left-3 top-1/2 -translate-y-1/2 w-1 h-6 bg-amber-300 rounded-full shadow-md shadow-amber-300/50"></span>
                        @endif
                        <x-icon :name="$item['icon']"
                            class="h-5 w-5 shrink-0 {{ $active ? 'text-brand-700' : 'text-white/70 group-hover:text-white' }}" />
                        <span class="whitespace-nowrap transition-all duration-300"
                            :class="!sidebarOpen && 'lg:opacity-0 lg:w-0 lg:hidden'">{{ $item['label'] }}</span>
                    </a>
                @endforeach
            </nav>
            <div class="relative z-10 p-3 border-t border-white/15 backdrop-blur-sm">
                <a href="{{ route('home') }}" target="_blank"
                    class="flex items-center gap-3 rounded-xl px-3 py-2 text-xs text-white/70 hover:bg-white/15 hover:text-white apple-transition"
                    :title="!sidebarOpen ? 'Lihat Situs Publik' : ''">
                    <x-icon name="globe" class="h-4 w-4 shrink-0" />
                    <span class="whitespace-nowrap transition-all duration-300"
                        :class="!sidebarOpen && 'lg:opacity-0 lg:w-0 lg:hidden'">Lihat Situs Publik</span>
                </a>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-w-0 transition-all duration-400 ease-ios" :class="sidebarOpen ? '' : ''">
            ...
            <header class="sticky top-0 z-30 lg:mt-4 mx-4 lg:mx-8 mb-4">
                <div
                    class="liquid-glass rounded-[2rem] flex items-center justify-between px-4 sm:px-6 h-16 gap-3 apple-transition hover:shadow-md">
                    <div class="flex items-center gap-2 min-w-0">
                        ...
                        <h1 class="font-bold text-slate-900 text-base sm:text-lg truncate drop-shadow-sm">
                            {{ $title ?? '' }}</h1>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('home') }}" target="_blank"
                            class="hidden md:inline-flex px-3 py-1.5 rounded-full bg-white/50 text-xs font-semibold text-slate-600 hover:text-brand-600 hover:bg-white apple-transition border border-white/60">Lihat
                            Situs ↗</a>
                        ...
                        <div class="relative">
                            <button @click="profileOpen = !profileOpen" @click.outside="profileOpen = false"
                                ...
                                <img src="{{ $user->photoUrl() }}"
                                    class="h-8 w-8 rounded-full object-cover ring-2 ring-white/80 shadow-sm"
                                    alt="">
                                <div class="hidden sm:block text-left text-xs leading-tight">
                                    <div class="font-semibold text-slate-900 truncate max-w-[120px] drop-shadow-sm">
                                        {{ $user->name }}</div>
                                    <div class="text-slate-500 font-medium">{{ ucfirst($user->role) }}</div>
                                </div>
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="hidden sm:block h-4 w-4 text-slate-500 transition-transform ease-ios duration-200"
                            ...
                            <div x-show="profileOpen" x-cloak x-transition:enter="ease-ios duration-200"
                                ...
                                <div class="px-3 py-2 border-b border-white/60 mb-1">
                                    <div class="text-sm font-semibold text-slate-900 truncate">{{ $user->name }}
                                    </div>
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="block mt-1">@csrf
                                    <button
                                        class="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50/80 rounded-xl apple-transition text-left">
            ...
            <main class="px-4 sm:px-6 lg:px-8 pb-8 flex-1">
                <div id="panel-page-content" class="animate-page-in" x-data x-init="$el.classList.remove('animate-page-in');
