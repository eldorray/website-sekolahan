...
@php
    ...
    $val = $value;
@endphp
<div data-tiptap-root
    class="rounded-xl border border-slate-200 bg-white focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-200 transition"
    wire:ignore>
    <div class="flex flex-wrap gap-1 border-b border-slate-100 p-1.5 bg-slate-50/60 rounded-t-xl text-xs">
        ...
        <button type="button" data-tiptap-action="strike" class="{{ $btn }} line-through"
        ...
        <button type="button" data-tiptap-action="ordered" class="{{ $btn }}"
            title="Numbered list">1.</button>
        <button type="button" data-tiptap-action="blockquote" class="{{ $btn }}" title="Quote">❝</button>
        <button type="button" data-tiptap-action="code" class="{{ $btn }} font-mono"
    ...
    <div data-tiptap-editor data-placeholder="{{ $placeholder }}" class="min-h-[180px] tiptap-fallback"></div>
    <input type="hidden" data-tiptap-input @if ($model) wire:model="{{ $model }}" @endif
        value="{{ $val }}" />
</div>
