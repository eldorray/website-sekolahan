<div>
    <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm responsive-table">
                    ...
                    <tbody class="divide-y">
                        @foreach ($items as $u)
                            <tr>
                                <td data-label="Nama" class="py-2.5">{{ $u->name }}</td>
                                <td data-label="Email" class="text-xs break-all">{{ $u->email }}</td>
                                <td data-label="Role"><span
                                        class="rounded bg-slate-100 px-2 py-0.5 text-xs">{{ $u->role }}</span></td>
        ...
        <div class="card">
            ...
            <form wire:submit="save" class="space-y-3">
                ...
                <div><label class="label">Password {{ $editingId ? '(opsional)' : '' }}</label><input type="password"
                        wire:model="password" class="input">
                    @error('password')
                        <p class="text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div><label class="label">Role</label><select wire:model="role" class="select">
                        <option value="admin">Admin</option>
                        <option value="guru">Guru</option>
                    </select></div>
                <label class="inline-flex items-center gap-2 text-sm"><input type="checkbox" wire:model="is_active">
                    Aktif</label>
                <div class="flex gap-2"><button class="btn-primary">Simpan</button>
                    @if ($editingId)
    ...
    <x-confirm-delete title="Hapus Pengguna?" description="Akun pengguna ini akan dihapus permanen." :show="(bool) $confirmingDeleteId"
