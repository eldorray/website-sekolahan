<div>
    <div class="grid lg:grid-cols-3 gap-6">
                            ...
                            <tr>
                                <td data-label="Judul" class="py-2.5">
                                    <div>
                                        ...
                                        <div class="text-xs text-slate-500">{{ $p->short_description }}</div>
                                    </div>
                                </td>
                                <td data-label="Order">{{ $p->order }}</td>
                                ...
                                <td data-label="Aksi" class="text-right">
                                    ...
                                    <button
                                        wire:click="confirmDelete({{ $p->id }}, @js($p->title))"
                                        class="text-red-600 hover:underline text-xs ml-2">Hapus</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-6 text-slate-500">Belum ada program.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="card">
            ...
            <form wire:submit="save" class="space-y-3">
                <div><label class="label">Judul</label><input wire:model="title" class="input">
                    @error('title')
                        <p class="text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div><label class="label">Icon (nama)</label><input wire:model="icon" class="input"></div>
                <div><label class="label">Deskripsi Singkat</label>
                    <textarea wire:model="short_description" rows="2" class="textarea"></textarea>
                ...
                <div><label class="label">Gambar</label><input type="file" wire:model="image_file" accept="image/*"
                        class="text-xs">
                    @error('image_file')
                        <p class="text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div><label class="label">Urutan</label><input type="number" wire:model="order" class="input">
                    </div>
                    <div class="flex items-end"><label class="inline-flex items-center gap-2 text-sm"><input
                                type="checkbox" wire:model="is_active"> Aktif</label></div>
                </div>
    ...
    <x-confirm-delete title="Hapus Program?" description="Program ini akan dihapus permanen." :show="(bool) $confirmingDeleteId"
