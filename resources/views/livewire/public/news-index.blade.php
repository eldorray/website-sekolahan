<div>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div
                ...
                <div
                    ...
                    {{ __('Informasi Terkini') }}
                </div>
                <h1 class="text-4xl sm:text-5xl font-extrabold text-slate-900 drop-shadow-sm">{{ __('Berita') }} & <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-brand-500 to-brand-700">{{ __('Artikel') }}</span>
                </h1>
            </div>
        </div>
    </section>
    <section class="py-12 animate-fade-up delay-100">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($items as $n)
                <a href="{{ route('news.show', $n->slug) }}" wire:navigate
                    ...
                    <div
                        ...
                        <span
                            class="absolute top-3 left-3 rounded-full bg-white/90 backdrop-blur-sm px-3 py-1 text-[10px] font-bold text-brand-600 shadow-sm uppercase tracking-wider">{{ $n->category }}</span>
                    </div>
                    <div class="flex-grow px-2">
                        ...
                        <h3
                            class="font-bold text-slate-900 text-lg group-hover:text-brand-600 transition-colors duration-300 mb-2 line-clamp-2 leading-snug">
                            {{ $n->title }}</h3>
                        <p class="text-[13px] font-medium text-slate-600 line-clamp-2 leading-relaxed">
                            {{ $n->excerpt }}</p>
                    </div>
                </a>
            @endforeach
