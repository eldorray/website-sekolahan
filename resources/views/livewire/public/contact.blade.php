...
<div>
    <section class="py-8 animate-fade-up">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div
                ...
                <h1 class="text-4xl sm:text-5xl font-extrabold text-slate-900 drop-shadow-sm">{{ __('Hubungi') }} <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-brand-500 to-brand-700">{{ __('Sekolah') }}</span>
                </h1>
            </div>
        </div>
    </section>
    <section class="py-12 animate-fade-up delay-100">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-8 items-start">
            <div class="space-y-8">
                <div
                    class="liquid-glass rounded-[2rem] p-8 border border-white/80 shadow-sm apple-transition hover:-translate-y-1 hover:shadow-md">
                    ...
                    <div class="space-y-4 text-[14px] font-medium text-slate-600">
                        <div class="flex gap-4 p-3 rounded-xl hover:bg-white/50 transition-colors"><span
                                class="font-bold w-24 text-slate-900 shrink-0">{{ __('Alamat') }}</span><span>{{ Setting::get('address') }}</span>
                        </div>
                        <div class="flex gap-4 p-3 rounded-xl hover:bg-white/50 transition-colors"><span
                                class="font-bold w-24 text-slate-900 shrink-0">{{ __('Telepon') }}</span><span>{{ Setting::get('phone') }}</span>
                        </div>
                        <div class="flex gap-4 p-3 rounded-xl hover:bg-white/50 transition-colors"><span
                                class="font-bold w-24 text-slate-900 shrink-0">Email</span><span>{{ Setting::get('email') }}</span>
                        </div>
                        <div class="flex gap-4 p-3 rounded-xl hover:bg-white/50 transition-colors"><span
                                class="font-bold w-24 text-slate-900 shrink-0">Website</span><span>{{ Setting::get('website') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div
                class="liquid-glass rounded-[2rem] p-8 border border-white/80 shadow-sm apple-transition hover:-translate-y-1 hover:shadow-md h-full">
                <h2 class="text-2xl font-bold mb-6 text-slate-900 flex items-center gap-3">
