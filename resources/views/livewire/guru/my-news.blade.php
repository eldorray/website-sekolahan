...
<div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 card">
        <h2 class="font-bold text-slate-900 mb-3">Berita Saya</h2>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="text-left text-slate-500 border-b">
                    <tr><th class="py-2">Judul</th><th>Kategori</th><th>Status</th><th class="text-right">Aksi</th></tr>
                </thead>
                <tbody class="divide-y">
                    @forelse ($items as $n)
                        <tr>
                            <td class="py-2.5">{{ Str::limit($n->title, 50) }}</td>
                    ...
                    @empty
                        <tr><td colspan="4" class="py-6 text-center text-slate-500">Belum ada berita.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="card">
        ...
        <form wire:submit="save" class="space-y-3">
            ...
            <div><label class="label">Kategori</label><select wire:model="category" class="select"><option>KEGIATAN</option><option>PRESTASI</option><option>ARTIKEL</option><option>PENGUMUMAN</option></select></div>
            ...
            <div><label class="label">Gambar</label><input type="file" wire:model="image_file" class="text-xs">@error('image_file')<p class="text-xs text-red-500">{{ $message }}</p>@enderror</div>
